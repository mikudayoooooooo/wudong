...
#   网络 → 数据库容器 → compose 三服务 → DB_SYNC 建表 → 灌演示数据 → 灌管理端菜单 → 自检
...
step "3/7 起服务（midway / admin / web）"
docker compose up -d

step "4/7 建表（DB_SYNC，仅首次）"
...
RESULT=$(docker exec wudong-mysql mysql -N -u"$DB_USER" -p"$DB_PASS" "$DB_NAME" -e "SELECT
 ...
 (SELECT COUNT(*) FROM base_sys_menu WHERE viewPath LIKE 'modules/%')" 2>/dev/null | tr '\t' ' ' | tr -d '\n')
read -r HOTEL REST ROUTE INV SLOT MENU <<< "$RESULT"
check(){ if [ "${2:-0}" -ge "$3" ] 2>/dev/null; then printf "  ✅ %s：%s\n" "$1" "$2"; else printf "  ❌ %s：%s（期望 ≥%s）——重跑本脚本或查上方报错\n" "$1" "${2:-无}" "$3"; fi; }
check "民宿（含云上人家）" "$HOTEL" 3
check "餐厅（长桌宴）" "$REST" 1
check "路线" "$ROUTE" 2
check "国庆路线库存" "$INV" 1
check "长桌宴时段" "$SLOT" 2
