...
顶栏入口在**已登录且已加载到店铺信息**时显示店铺名（判据是店铺名非空，**不看店铺状态** —— 被禁用但已入驻的商家登录后照样显示自己的店名）。
此处**刻意不做全局预取**：新标签页直接打开 C 端首页时可能先显示通用文案「商家中心」，
进入商家区或登录后即变为店铺名。这样可避免为一句顶栏文案给所有 C 端公开页各加一次鉴权请求，
也避免 token 过期时把正在浏览 C 端的访客顺带踢到登录页。
